...
<Tabs syncKey="runtime">
  ...
  <TabItem label="Python" icon="seti:python">
    ...
    jev_classifier = JevClassifier(JevClassifierOptions(
        ...
        # Only the most recent messages of the history are sent. Defaults to 20; None keeps all.
...
- `maxHistoryMessages` (optional): How many of the most recent history messages are sent to Jev (Python: `max_history_messages`). Defaults to 20 (10 exchanges); `null` (Python: `None`) sends the full history, and `0` sends none. See [Limitations](#limitations) for why the history is bounded.
...
6. **Bound the history**: the default `maxHistoryMessages` of 20 is enough for follow-ups that answer a question from a few turns back. Raise it only if your agents' questions stay open longer (see [Limitations](#limitations)).
...
- **Context length**: a request is limited to [64k tokens, and the state plus the longest question to 32k tokens](https://docs.typesafe.ai/models). The classifier sends only the last `maxHistoryMessages` messages (20 by default) of the session history, which keeps typical sessions well within the limit. Very long individual messages, or a raised or disabled `maxHistoryMessages`, can still exceed it and fail with a `422`. Accuracy also [drops as the state grows](https://docs.typesafe.ai/model-jaggedness/jev-1.13#large-state-full-of-irrelevant-detail) long before that.
