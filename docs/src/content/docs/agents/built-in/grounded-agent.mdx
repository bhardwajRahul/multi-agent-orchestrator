...
In TypeScript, the widget arrives as a `{ ui }` object chunk interleaved with the text-string chunks — whether you iterate the **agent's** stream directly or route through `AgentSquad.routeRequest` (the orchestrator forwards the widget chunk and keeps it out of the saved text answer). Check each chunk's type as you consume the stream. (Python delivers it the same way, as `AgentStreamResponse.ui`.)
